...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['login'])) {
    ...
    $cashier = $db->cashierLogin($username, $password);

    if ($cashier) {
        $_SESSION['cashier_ID'] = $cashier['cashier_ID'];
        $_SESSION['cashier_FN'] = $cashier['cashier_FN'];
        header("Location: cashier_home.php");
        exit();
    } else {
        $alert = "<script>Swal.fire('Login Failed', 'Invalid username or password.', 'error');</script>";
    }
}
?>
...
<head>
<meta charset="UTF-8">
<title>Cashier Login | Izana</title>
